...
BEGIN
  ...
  END IF;
  IF (SELECT count(*) FROM tanaghom.agent_workflow_registry) <> 8 THEN
    RAISE EXCEPTION 'agent registry must expose eight specialized workflow workers';
  END IF;
  IF EXISTS (
    SELECT role.code
  ...
  IF (SELECT count(*) FROM tanaghom.agent_workflow_registry WHERE runtime_state='imported_inactive') <> 4
    OR (SELECT count(*) FROM tanaghom.agent_workflow_registry WHERE runtime_state='available_not_imported') <> 4
  ...
  IF EXISTS (
    SELECT 1 FROM tanaghom.agent_workflow_registry
     WHERE release_state<>'available' OR runtime_verified_at IS NULL
       OR runtime_evidence NOT IN ('production-audit-after-pr-83','implementation-merged-not-imported')
  ) THEN
  ...
  IF NOT EXISTS (SELECT 1 FROM tanaghom.agent_workflow_registry WHERE 'content.postiz.draft'=ANY(job_types))
    ...
    OR NOT EXISTS (SELECT 1 FROM tanaghom.agent_workflow_registry WHERE 'conversation.ghl.inbound_event'=ANY(job_types))
  THEN
