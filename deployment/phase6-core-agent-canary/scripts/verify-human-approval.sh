...
grep -Fx "CAMPAIGN=$TANAGHOM_CANARY_CAMPAIGN" "$evidence/canary.env" >/dev/null || die 'campaign evidence mismatch'
grep -Fx "PRODUCTION_COMMIT=$TANAGHOM_EXPECTED_PRODUCTION_COMMIT" "$evidence/canary.env" >/dev/null || die 'production commit evidence mismatch'
grep -Fx "SOURCE_COMMIT=$TANAGHOM_CANARY_SOURCE_COMMIT" "$evidence/canary.env" >/dev/null || die 'source commit evidence mismatch'
...
echo 'PASS: every canary draft has an authenticated active-human approval and no publishing or CRM action occurred.'
