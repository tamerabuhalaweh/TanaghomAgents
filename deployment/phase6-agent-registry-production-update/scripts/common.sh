...
EXPECTED_START_MIGRATION=0021_quality_baseline_shadow_pipeline
TARGET_MIGRATION=0022_agent_registry
PENDING_MIGRATIONS='0022_agent_registry'
...
db_scalar() {
  url=$(database_url)
  PGAPPNAME=tanaghom-phase6-agent-registry-release psql "$url" -X -v ON_ERROR_STOP=1 -At -c "$1"
  unset url
}
...
db_file() {
  url=$(database_url)
  status=0
  PGAPPNAME=tanaghom-phase6-agent-registry-release psql "$url" -X -v ON_ERROR_STOP=1 -f "$1" || status=$?
  unset url
  return "$status"
}
