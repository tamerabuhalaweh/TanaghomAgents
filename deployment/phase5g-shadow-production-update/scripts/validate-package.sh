...
grep -q 'docker exec -u node.*rm -f.*workflow_remote' "$package/scripts/deploy-update.sh"
grep -q 'ROLLBACK_CLEANUP_FAILED=YES' "$package/scripts/deploy-update.sh"
! grep -q 'docker exec -u root.*rm -f.*workflow_remote' "$package/scripts/deploy-update.sh"
