...
test "$(docker exec -u node "$N8N_MAIN_CONTAINER" n8n --version)" = "$N8N_EXPECTED_VERSION" || die 'n8n version differs from the reviewed workflow runtime'
docker exec "$N8N_DATABASE_CONTAINER" sh -c 'test -n "$POSTGRES_USER" && test -n "$POSTGRES_DB"' || die 'n8n database container metadata is unavailable'
assert_workflow_absent

echo "PASS: Phase 5G baseline-shadow production preflight passed for $TANAGHOM_RELEASE_ID."
