...
docker run -d --name "$postgres_container" --network "$network" -e POSTGRES_PASSWORD=postgres -e POSTGRES_DB=n8n_shadow_test "$postgres_image" >/dev/null
attempt=0
...
run_n8n() {
  docker run --rm --network "$network" \
    ...
    -e DB_TYPE=postgresdb -e DB_POSTGRESDB_HOST="$postgres_container" -e DB_POSTGRESDB_PORT=5432 \
...
docker run -d --name "$n8n_container" --network "$network" \
  -e N8N_ENCRYPTION_KEY=integration-only-encryption-key-32 \
  -e DB_TYPE=postgresdb -e DB_POSTGRESDB_HOST="$postgres_container" -e DB_POSTGRESDB_PORT=5432 \
  -e DB_POSTGRESDB_DATABASE=n8n_shadow_test -e DB_POSTGRESDB_USER=postgres -e DB_POSTGRESDB_PASSWORD=postgres \
