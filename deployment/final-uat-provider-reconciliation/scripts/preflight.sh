...
test -z "$(git -C "$RELEASE_SOURCE_ROOT" diff --name-only \
  "$TANAGHOM_EXPECTED_CURRENT_COMMIT..$TANAGHOM_TARGET_COMMIT" -- \
...
test "$(db_scalar 'SELECT version FROM public.schema_migrations ORDER BY applied_at DESC LIMIT 1;')" = \
  "$EXPECTED_MIGRATION" ||
...
echo "PASS: Final UAT provider-reconciliation preflight passed for $TANAGHOM_PROVIDER_RECONCILIATION_ID."
