...
rollback_image="tanaghom-dashboard-canary:rollback-$TANAGHOM_PROVIDER_RECONCILIATION_ID"
...
capture_reconciliation_firewall "$evidence/firewall.before"
...
provider_state_fingerprint > "$evidence/provider-state.before"
...
RELEASE_ID=$TANAGHOM_PROVIDER_RECONCILIATION_ID
...
automatic_rollback() {
  ...
  if test "$source_changed" = true; then
    git -C "$PRODUCTION_ROOT" -c safe.directory="$PRODUCTION_ROOT" \
      checkout --detach "$TANAGHOM_EXPECTED_CURRENT_COMMIT" >/dev/null
  fi
...
test "$(git -C "$PRODUCTION_ROOT" -c safe.directory="$PRODUCTION_ROOT" rev-parse FETCH_HEAD)" = \
...
test -z "$(production_unexpected_changes)" ||
  die 'production checkout is dirty after target checkout'
...
echo "PASS: Final UAT provider allowlist committed. Evidence: $evidence"
