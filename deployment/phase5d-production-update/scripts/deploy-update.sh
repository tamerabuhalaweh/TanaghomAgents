...
chmod 0600 "$evidence_dir/target-commit.txt" "$evidence_dir/up-migrations.sha256"
...
automatic_rollback() {
  ...
  set +e
  rollback_failed=0
  echo 'Release did not commit; restoring the previous Tanaghom dashboard and package-applied migrations.' >&2
  ...
  if test "$rollback_failed" -ne 0; then
    ...
    echo "ERROR: automatic rollback was incomplete; keep emergency stops active and follow the recovery runbook." >&2
  fi
}
...
echo "PASS: Phase 5D production update committed. Evidence: $evidence_dir"
