...
for file in README.md RUNBOOK.md scripts/common.sh scripts/preflight.sh scripts/run-canary.sh scripts/restore-workflows.sh scripts/verify-human-approval.sh scripts/validate-package.sh scripts/test-refusal-paths.sh scripts/test-disposable-lifecycle.sh scripts/existing-campaign-operator.mjs; do
...
grep -q 'SET LOCAL ROLE tanaghom_api' "$package/scripts/existing-campaign-operator.mjs"
...
if grep -E 'operator (seed|mark-failed)|INSERT INTO tanaghom\.(campaigns|agent_jobs)' "$package/scripts/existing-campaign-operator.mjs" "$package/scripts/run-canary.sh"; then echo 'runtime package may not seed, fail, or directly insert campaign jobs' >&2; exit 1; fi
if grep -F '| tee' "$package/scripts/run-canary.sh" "$package/scripts/verify-human-approval.sh"; then echo 'a critical canary command exit can be masked by tee' >&2; exit 1; fi
