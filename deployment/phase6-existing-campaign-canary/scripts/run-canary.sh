...
cleanup() {
  ...
  if test "$status" -ne 0; then
    ...
    echo 'The existing campaign and jobs were deliberately preserved for human investigation.' >>"$evidence/automatic-restore.log"
  fi
  exit "$status"
}
...
operator verify-authorized "$TANAGHOM_CANARY_CAMPAIGN_ID" "$TANAGHOM_CANARY_STRATEGY_JOB_ID" "$TANAGHOM_CANARY_CAMPAIGN" "$TANAGHOM_EXPECTED_CONTENT_ITEMS" >"$evidence/authorized-baseline.json"
...
import_workflow_inactive "$evidence/$STRATEGIST_ID.canary.json" strategist-canary
...
# Reassert identity immediately before the generic n8n claim function becomes reachable.
...
test "$(workflow_active "$STRATEGIST_ID")" = 1 || die 'strategist did not enter the active state'
operator verify-authorized "$TANAGHOM_CANARY_CAMPAIGN_ID" "$TANAGHOM_CANARY_STRATEGY_JOB_ID" "$TANAGHOM_CANARY_CAMPAIGN" "$TANAGHOM_EXPECTED_CONTENT_ITEMS" >/dev/null
execute_workflow_once "$STRATEGIST_ID" >"$evidence/strategist-execution.json"
unpublish_workflow "$STRATEGIST_ID"
set_registry_inactive "$STRATEGIST_REGISTRY"
assert_workflow_inactive "$STRATEGIST_ID"
operator verify-strategy "$TANAGHOM_CANARY_CAMPAIGN_ID" "$TANAGHOM_CANARY_STRATEGY_JOB_ID" "$TANAGHOM_CANARY_CAMPAIGN" "$TANAGHOM_EXPECTED_CONTENT_ITEMS" >"$evidence/strategy-result.json"
...
echo 'PASS: the exact existing zero-budget UAT campaign reached human approval. Both workflows are restored inactive; publishing and CRM remained untouched.'
