...
WORKFLOW_CONTRACT="$RELEASE_SOURCE_ROOT/deployment/phase6-core-agent-canary/scripts/workflow-contract.mjs"
...
is_uuid() { printf '%s' "$1" | grep -Eq '^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$'; }
...
require_environment() {
  ...
  is_uuid "${TANAGHOM_CANARY_CAMPAIGN_ID:-}" || die 'campaign ID must be a canonical lowercase UUID'
  is_uuid "${TANAGHOM_CANARY_STRATEGY_JOB_ID:-}" || die 'strategy job ID must be a canonical lowercase UUID'
  case "${TANAGHOM_EXPECTED_CONTENT_ITEMS:-}" in ''|*[!0-9]*) die 'expected content items must be an integer' ;; esac
  test "$TANAGHOM_EXPECTED_CONTENT_ITEMS" -ge 1 && test "$TANAGHOM_EXPECTED_CONTENT_ITEMS" -le 12 || die 'expected content items must be between 1 and 12'
...
db_scalar() { url=$(database_url); PGAPPNAME=tanaghom-existing-campaign-canary psql "$url" -X -v ON_ERROR_STOP=1 -At -c "$1"; unset url; }
db_exec() { url=$(database_url); PGAPPNAME=tanaghom-existing-campaign-canary psql "$url" -X -v ON_ERROR_STOP=1 -c "$1" >/dev/null; unset url; }
...
export_all_workflows() {
  destination=$1; remote="/home/node/tanaghom-existing-canary-export-$$.json"
...
operator() {
  ...
  DATABASE_URL=$(database_url) NODE_EXTRA_CA_CERTS="$DATABASE_CA_CERT" TANAGHOM_DATABASE_SSL_MODE=verify-full \
    node "$SCRIPT_DIR/existing-campaign-operator.mjs" "$@"
}
