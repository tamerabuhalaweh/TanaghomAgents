...
test "$(git -C "$RELEASE_SOURCE_ROOT" rev-parse HEAD)" = "$TANAGHOM_CANARY_SOURCE_COMMIT" || die 'resume package checkout differs from the approved source commit'
test -z "$(git -C "$RELEASE_SOURCE_ROOT" status --porcelain --untracked-files=no)" || die 'resume package checkout has tracked modifications'
...
echo 'PASS: the exact persisted strategy is ready for a Content Producer-only resume; no state was changed.'
