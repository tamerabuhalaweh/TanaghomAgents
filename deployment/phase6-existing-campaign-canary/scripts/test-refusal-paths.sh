...
if env $base TANAGHOM_CANARY_AUTHORIZATION=YES-I-AM-THE-AUTHORIZED-OWNER TANAGHOM_CANARY_ALLOW_OWNER_FUNCTION_CALL=wrong sh -c '. "$1/common.sh"; require_environment' sh "$script" >/dev/null 2>&1; then echo 'invalid privileged function-call authorization was accepted' >&2; exit 1; fi
...
echo 'PASS: owner and privileged-call authorization, release ID, campaign identity, .test suffix, and content-target refusal paths are fail-closed.'
