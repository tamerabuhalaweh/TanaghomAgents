...
EXPECTED_START_MIGRATION=0014_supervised_conversation_ownership
TARGET_MIGRATION=0019_notification_monitoring_destinations
PENDING_MIGRATIONS='0015_governed_ghl_actions 0016_ghl_action_review_reconciliation 0017_ghl_service_action_audit_attribution 0018_conversation_capacity_backpressure 0019_notification_monitoring_destinations'
...
require_release_environment() {
  ...
  case "${TANAGHOM_RELEASE_ID:-}" in
    phase5f-[0-9][0-9][0-9][0-9][0-9][0-9][0-9][0-9]T[0-9][0-9][0-9][0-9][0-9][0-9]Z) ;;
    *) die 'TANAGHOM_RELEASE_ID must use phase5f-YYYYMMDDTHHMMSSZ' ;;
  esac
...
db_scalar() {
  url=$(database_url)
  PGAPPNAME=tanaghom-phase5f-release psql "$url" -X -v ON_ERROR_STOP=1 -At -c "$1"
  unset url
}
...
db_file() {
  url=$(database_url)
  status=0
  PGAPPNAME=tanaghom-phase5f-release psql "$url" -X -v ON_ERROR_STOP=1 -f "$1" || status=$?
  unset url
  return "$status"
}
...
assert_release_tables_empty() {
  test "$(db_scalar "SELECT (SELECT count(*) FROM tanaghom.ghl_message_template_versions)+(SELECT count(*) FROM tanaghom.ghl_contact_channel_policies)+(SELECT count(*) FROM tanaghom.ghl_action_jobs)+(SELECT count(*) FROM tanaghom.ghl_action_outcomes)+(SELECT count(*) FROM tanaghom.ghl_action_approvals)+(SELECT count(*) FROM tanaghom.ghl_action_reconciliations)+(SELECT count(*) FROM tanaghom.conversation_dependency_cooldowns)+(SELECT count(*) FROM tanaghom.notification_destinations);")" = 0 || die 'new release tables contain customer, approval, or action data; automatic schema rollback is unsafe'
  test "$(db_scalar "SELECT count(*) FROM tanaghom.organization_crm_policies WHERE action_mode<>'manual' OR proactive_message_mode<>'disabled' OR action_emergency_stop IS NOT TRUE OR action_emergency_reason<>'Awaiting governed GHL action activation' OR cardinality(action_allowed_channels)<>0 OR action_quiet_hours_start<>time '21:00' OR action_quiet_hours_end<>time '08:00' OR action_timezone<>'UTC' OR action_contact_frequency_cap_24h<>2 OR action_policy_changed_by IS NOT NULL OR action_policy_changed_at IS NOT NULL;")" = 0 || die 'a GHL action policy differs from its release default; schema rollback is unsafe'
  test "$(db_scalar "SELECT count(*) FROM tanaghom.conversation_capacity_policies WHERE max_conversation_concurrency<>8 OR max_model_claims_per_minute<>600 OR max_ghl_action_concurrency<>4 OR max_ghl_actions_per_minute<>120 OR interactive_backlog_threshold<>100 OR queue_age_warning_seconds<>120 OR updated_by IS NOT NULL;")" = 0 || die 'a capacity policy differs from its release default; schema rollback is unsafe'
  test "$(db_scalar "SELECT count(*) FROM tanaghom.notification_delivery_controls WHERE runtime_ready IS NOT FALSE OR emergency_stop IS NOT TRUE OR reason<>'Notification delivery is disabled until the runtime and destination are approved';")" = 0 || die 'notification delivery controls differ from their release default; schema rollback is unsafe'
}
