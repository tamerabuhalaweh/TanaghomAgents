...
test "$(db_scalar "SELECT count(*) FROM tanaghom.quality_rollout_policies;")" = "$(db_scalar "SELECT count(*) FROM tanaghom.organizations;")" || die 'a quality rollout policy is missing'
...
echo 'PASS: Phase 5G release validation passed at baseline with zero quality evidence, decisions, or provider operations.'
