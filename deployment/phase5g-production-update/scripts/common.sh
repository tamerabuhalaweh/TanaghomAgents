...
EXPECTED_START_MIGRATION=0019_notification_monitoring_destinations
TARGET_MIGRATION=0020_quality_rollout_control
PENDING_MIGRATIONS='0020_quality_rollout_control'
...
require_release_environment() {
  ...
  case "${TANAGHOM_RELEASE_ID:-}" in
    phase5g-[0-9][0-9][0-9][0-9][0-9][0-9][0-9][0-9]T[0-9][0-9][0-9][0-9][0-9][0-9]Z) ;;
    *) die 'TANAGHOM_RELEASE_ID must use phase5g-YYYYMMDDTHHMMSSZ' ;;
  esac
...
db_scalar() {
  url=$(database_url)
  PGAPPNAME=tanaghom-phase5g-release psql "$url" -X -v ON_ERROR_STOP=1 -At -c "$1"
  unset url
}
...
db_file() {
  url=$(database_url)
  status=0
  PGAPPNAME=tanaghom-phase5g-release psql "$url" -X -v ON_ERROR_STOP=1 -f "$1" || status=$?
  unset url
  return "$status"
}
...
assert_quality_tables_safe_to_drop() {
  test "$(db_scalar "SELECT (SELECT count(*) FROM tanaghom.quality_evaluation_snapshots)+(SELECT count(*) FROM tanaghom.quality_rollout_decisions);")" = 0 || die 'quality evidence or rollout decisions exist; automatic schema rollback is unsafe'
  test "$(db_scalar "SELECT count(*) FROM tanaghom.quality_rollout_policies WHERE current_stage<>'baseline';")" = 0 || die 'an organization has left the quality baseline; automatic schema rollback is unsafe'
}
