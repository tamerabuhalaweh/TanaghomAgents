...
evidence=$1
test -d "$evidence" || die 'resume evidence directory is missing'
test ! -e "$evidence/probe-result.env" || die 'Arabic probe already completed'
...
url=$(database_url)
PGAPPNAME=tanaghom-arabic-probe psql "$url" -X -v ON_ERROR_STOP=1 -At -c "
...
" >"$claimed"
unset url
test -s "$claimed" || die 'terminal Arabic probe source job is missing'
node \
  "$RELEASE_SOURCE_ROOT/deployment/phase6-bilingual-uat-completion/scripts/probe-contract.mjs" \
  build "$STRATEGIST_SOURCE" "$claimed" "$request"
...
test "$http_status" = 200 || die "Arabic Gemma probe returned HTTP $http_status"
node \
  "$RELEASE_SOURCE_ROOT/deployment/phase6-bilingual-uat-completion/scripts/probe-contract.mjs" \
  validate "$response"
...
test "$(systemctl is-active "$GEMMA_UNIT")" = active ||
  die 'Gemma became inactive during the Arabic probe'
test "$after_pid" = "$before_pid" || die 'Gemma process changed during the Arabic probe'
test "$after_started" = "$before_started" ||
  die 'Gemma start time changed during the Arabic probe'
...
then
  die 'Gemma/xgrammar fatal log occurred during the Arabic probe'
fi
...
echo 'PASS: bounded Arabic Gemma probe completed without truncation or engine restart.'
