...
# The dashboard must remain outside every n8n network.
...
(async () => {
  for (const allowed of ['api.thesmartlabs.net', gateway]) {
    const result = await tunnel(allowed);
    console.log(`proxy approved ${allowed}:443 HTTP ${result.status}`);
    result.socket?.destroy();
    if (result.status !== 200) process.exit(71);
  }
  ...
  const direct = await Promise.all([
    socketConnect('38.247.187.232', 443), socketConnect('10.0.0.1', 5432),
  ...
  console.log('PASS: exact-hostname proxy, TLS, and gateway authentication boundary enforced.');
