...
grep -q 'assert_production_worktree_unchanged' "$package/scripts/validate-release.sh"
grep -q 'import:credentials' "$package/scripts/deploy-update.sh"
grep -q 'docker exec -i -u node.*cat >.*credential_remote' "$package/scripts/deploy-update.sh"
