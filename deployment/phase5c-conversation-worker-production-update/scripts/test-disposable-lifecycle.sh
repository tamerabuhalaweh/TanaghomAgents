...
test "$(scalar "WITH updated AS (UPDATE tanaghom.agent_workflow_registry SET runtime_state='imported_inactive',runtime_evidence='disposable-test' WHERE code='conversation_intelligence_worker' AND runtime_state='available_not_imported' RETURNING 1) SELECT count(*) FROM updated;")" = 1
...
test "$(scalar "WITH updated AS (UPDATE tanaghom.agent_workflow_registry SET runtime_state='available_not_imported',trigger_state='disabled' WHERE code='conversation_intelligence_worker' AND runtime_state='imported_inactive' RETURNING 1) SELECT count(*) FROM updated;")" = 1
