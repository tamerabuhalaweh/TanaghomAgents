...
automatic_rollback() {
  ...
  if test "$(latest_migration 2>/dev/null)" = "$TARGET_MIGRATION"; then
    assert_agent_registry_safe_to_drop >/dev/null 2>&1 || rollback_failed=1
...
echo "PASS: Phase 6 Campaign Lifecycle production update committed. Evidence: $evidence_dir"
