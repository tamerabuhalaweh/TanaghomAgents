...
test "$(db_scalar "SELECT count(*) FROM information_schema.columns WHERE table_schema='tanaghom' AND table_name='campaigns' AND column_name='content_item_target' AND data_type='integer' AND is_nullable='NO';")" = 1 || die 'campaign content target column is missing or nullable'
test "$(db_scalar "SELECT count(*) FROM tanaghom.campaigns WHERE content_item_target NOT BETWEEN 1 AND 12;")" = 0 || die 'a campaign has an invalid content target'
test "$(db_scalar "SELECT count(*) FROM pg_indexes WHERE schemaname='tanaghom' AND indexname='agent_jobs_one_open_core_job_per_campaign_idx' AND indexdef LIKE 'CREATE UNIQUE INDEX%';")" = 1 || die 'open core-job uniqueness index is missing'
for signature in \
  ...
  test "$(db_scalar "SELECT has_function_privilege('tanaghom_api','$signature','EXECUTE');")" = t || die "dashboard API cannot execute $signature"
  test "$(db_scalar "SELECT has_function_privilege('tanaghom_n8n_worker','$signature','EXECUTE');")" = f || die "n8n received campaign-control access to $signature"
  test "$(db_scalar "SELECT count(*) FROM pg_proc p, LATERAL aclexplode(coalesce(p.proacl, acldefault('f',p.proowner))) acl WHERE p.oid='$signature'::regprocedure AND acl.grantee=0 AND acl.privilege_type='EXECUTE';")" = 0 || die "PUBLIC received campaign-control access to $signature"
done
...
test "$(curl -sS -o /dev/null -w '%{http_code}' --max-time 15 "https://$PUBLIC_HOST/campaigns")" = 307 || die 'Campaigns page authentication boundary changed'
test "$(curl -sS -o /dev/null -w '%{http_code}' --max-time 15 "https://$PUBLIC_HOST/campaigns/00000000-0000-4000-8000-000000000000")" = 307 || die 'campaign detail authentication boundary changed'
test "$(curl -sS -o /dev/null -w '%{http_code}' --max-time 15 "https://$PUBLIC_HOST/api/campaigns")" = 401 || die 'campaign API authentication boundary changed'
test "$(curl -sS -o /dev/null -w '%{http_code}' --max-time 15 "https://$PUBLIC_HOST/api/campaigns/00000000-0000-4000-8000-000000000000")" = 401 || die 'campaign detail API authentication boundary changed'
...
echo 'PASS: Phase 6 Campaign Lifecycle release validation passed with governed campaign creation/detail controls, zero active workflows, and unchanged protected boundaries.'
