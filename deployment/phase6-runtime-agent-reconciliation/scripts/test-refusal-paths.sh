...
release_id=phase6-runtime-agents-20260721T120000Z
inherited=$(TANAGHOM_RELEASE_SOURCE_ROOT="$root" TANAGHOM_RUNTIME_AGENT_AUTHORIZATION=YES-I-AM-THE-AUTHORIZED-OWNER TANAGHOM_RUNTIME_AGENT_RELEASE_ID="$release_id" TANAGHOM_EXPECTED_PRODUCTION_COMMIT=aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa TANAGHOM_RUNTIME_AGENT_SOURCE_COMMIT=bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb sh -c '. "$1/common.sh"; require_runtime_agent_environment; printf %s "$TANAGHOM_RELEASE_ID"' sh "$script")
test "$inherited" = "$release_id" || { echo 'shared inventory helper release ID was not initialized' >&2; exit 1; }
