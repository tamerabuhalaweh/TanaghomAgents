...
test('security overrides keep audited URI and image dependencies on patched releases', async () => {
  ...
  assert.equal(manifest.devDependencies.postcss, '8.5.18');
...
test('Phase 3 Gemma contracts and prompts are versioned and non-operational', async () => {
  ...
  const names = [
    ...
    'strategist-output.v2.schema.json',
  ...
  const strategistV2 = await readFile(new URL('../prompts/campaign-strategist/v2.md', import.meta.url), 'utf8');
  ...
  assert.match(strategistV2, /do not return a separate channel list/);
...
test('Phase 5E GHL actions are governed, inactive, replay-safe, and least privileged', async () => {
  ...
  assert.match(runbook, /npm run db:rollback/);
  assert.match(runbook, /pg_restore --exit-on-error/);
  assert.match(quality, /test-disposable-backup\.sh "\$DATABASE_TEST_URL" 0033_agent_runtime_certification_evidence/);
  assert.match(quality, /TANAGHOM_EXPECTED_MIGRATION: 0033_agent_runtime_certification_evidence/);
...
test('Phase 6 Conversation Intelligence schema hotfix is exact, inactive-only, and reversible', async () => {
  ...
  assert.match(common, /EXPECTED_OLD_OPERATIONAL_SHA=f649acad1b0a8ef3a717d514bbe3ba0590865d4aadadad65df62df045b354ee9/);
  ...
  assert.match(contract, /explicit canonical enum enforcement/);
  assert.match(contract, /bounded canonical-enum and approved-citation normalizer/);
