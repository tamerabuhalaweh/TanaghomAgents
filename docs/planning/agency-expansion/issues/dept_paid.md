# Department expansion: paid-media planning before separately governed ad operations
...
The approved all-department roadmap includes 7 source profiles across paid-media. They are catalogued, not adapted, tested, available or activated. Provide campaign/ad-creative/measurement recommendations from approved evidence before considering any spending or ad-account operations.

## User story

As an organization owner, I want provide campaign/ad-creative/measurement recommendations from approved evidence before considering any spending or ad-account operations. with a clear capability boundary and no assumption that a role title grants authority.

## Scope

- Preserve coverage of all 7 pinned profiles in these divisions; the catalog is the authoritative per-profile checklist.
- Provide campaign/ad-creative/measurement recommendations from approved evidence before considering any spending or ad-account operations.
- Required data: Approved offers/assets, read-only campaign observations, declared attribution windows, customer budgets and source freshness.
- Review risks: Ad spend, unsupported performance guarantees, sensitive audience targeting, unauthorized tracking, misleading attribution and policy-sensitive creative.
- Initial execution boundary: Read-only planning/proposals. #156 owns the existing read-only Meta/Instagram MCP pilot; no bid/budget/audience/campaign mutation, pixel installation or automatic paid activation. Later writes require a separately approved spending and provider gate.
...
- #153 analytics
- #156 read-only MCP
- #136 connector governance
- #155 content variants
...
- [ ] All 7 assigned profiles have a reviewed disposition, primary delivery owner, source/skill versions, permissions, data classification, domain reviewer and next gate; none is silently omitted.
...
- [ ] Required representative cases include: Zero conversions, mixed attribution windows/currencies, missing consent, stale observations, unsafe audience request, budget-overrun proposal and rights/claim violations.
...
Use per-profile contract tests plus two-tenant authorization and adversarial tests, measured resource/latency limits, stop/replay/recovery evidence and qualified domain review where relevant. Zero conversions, mixed attribution windows/currencies, missing consent, stale observations, unsafe audience request, budget-overrun proposal and rights/claim violations. No live customer/provider activity is authorized by this planning issue.
...
Versioned planning snapshot: docs/planning/agency-expansion/issues/dept_paid.md. The snapshot is published through the planning documentation PR; issue/comment edits are not immutable release evidence.
