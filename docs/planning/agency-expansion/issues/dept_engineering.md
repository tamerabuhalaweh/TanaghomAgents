# Department expansion: engineering, security and testing teams
...
The approved all-department roadmap includes 80 source profiles across engineering, security, testing. They are catalogued, not adapted, tested, available or activated. Offer reviewed engineering and quality/security-assessment methods using customer-approved code or synthetic artifacts, separated from customer-facing business execution.

## User story

As an organization owner, I want offer reviewed engineering and quality/security-assessment methods using customer-approved code or synthetic artifacts, separated from customer-facing business execution. with a clear capability boundary and no assumption that a role title grants authority.

## Scope

- Preserve coverage of all 80 pinned profiles in these divisions; the catalog is the authoritative per-profile checklist.
- Offer reviewed engineering and quality/security-assessment methods using customer-approved code or synthetic artifacts, separated from customer-facing business execution.
- Required data: Explicitly authorized repositories, sanitized requirements, dependency manifests and isolated test outputs; no inherited server or organization-wide access.
- Review risks: Source secrets, supply-chain installation, dangerous shell/code, repository writes, deployment authority, intrusive security tests, production/network access and misleading security certification.
- Initial execution boundary: Read-only/code-proposal first. Any later code execution uses a separately certified disposable environment; network scans, penetration tests, destructive actions, deployments and repository writes require explicit scoped authority. Never access protected SmartLabs/SmartCC/voice systems.
...
- #136 approved connector boundary
- #137 certification
- #150 optional analyst, not deployment authority
...
- [ ] All 80 assigned profiles have a reviewed disposition, primary delivery owner, source/skill versions, permissions, data classification, domain reviewer and next gate; none is silently omitted.
...
- [ ] Required representative cases include: Secret-bearing source redaction, malicious repository instructions, unauthorized target/repo, dependency-install request, unsafe patch, false QA claim and sandbox escape attempt.
...
Use per-profile contract tests plus two-tenant authorization and adversarial tests, measured resource/latency limits, stop/replay/recovery evidence and qualified domain review where relevant. Secret-bearing source redaction, malicious repository instructions, unauthorized target/repo, dependency-install request, unsafe patch, false QA claim and sandbox escape attempt. No live customer/provider activity is authorized by this planning issue.
...
Versioned planning snapshot: docs/planning/agency-expansion/issues/dept_engineering.md. The snapshot is published through the planning documentation PR; issue/comment edits are not immutable release evidence.
