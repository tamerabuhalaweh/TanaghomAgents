...
import { getPostizPostAnalytics, validateProviderBaseUrl } from "@/lib/server/integration-providers";
...
export async function POST(request: NextRequest) {
  ...
  if (process.env.POSTIZ_PERFORMANCE_SYNC_ENABLED !== "true") {
    return noStore({ error: "postiz_performance_sync_disabled" }, { status: 503 });
  }
  ...
  if (typeof body.job_id !== "string" || !/^[0-9a-f-]{36}$/i.test(body.job_id)
      || !body.request_body || typeof body.request_body !== "object") {
    return noStore({ error: "invalid_gateway_request" }, { status: 400 });
  }
  const requestBody = body.request_body as Record<string, unknown>;
  if (typeof requestBody.provider_post_id !== "string" || requestBody.provider_post_id.length > 300
      || !Number.isInteger(requestBody.date) || Number(requestBody.date) < 1 || Number(requestBody.date) > 90) {
    return noStore({ error: "invalid_analytics_request" }, { status: 400 });
  }
  ...
  try {
    ...
    const result = await client.query<{
      ...
      `SELECT operation.id AS operation_id, connection.base_url, connection.credential_ciphertext,
        ...
        WHERE job.id = $1 AND job.job_type = 'postiz.performance.sync'
      ...
      [body.job_id, JSON.stringify(requestBody)],
    );
    ...
    const providerResponse = await getPostizPostAnalytics(
      baseUrl,
      secret,
      requestBody.provider_post_id,
      Number(requestBody.date),
    );
